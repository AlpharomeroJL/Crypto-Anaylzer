...
- Provider chain pattern implemented via registry + resilience; poll orchestrates fetches and writes.
...
**Failure modes**
- All providers down → LKG or explicit failure; detect via provider_health and doctor.
- Invalid or missing provenance → integrity warnings/failure depending on strict-integrity flag.
...
**Failure modes**
- Missing bars_1h when building bars_1D → explicit message; detect by running materialize and checking output.
- Empty or invalid snapshots → zero rows for affected pairs; detect via row counts.
...
**Failure modes**
- Singular or ill-conditioned design matrix → NaNs in betas/residuals; detect via NaN checks in downstream validation.
- Misaligned factor vs asset index → silent NaNs or wrong fit; detect via integrity checks on index alignment.
...
- ValidationBundle per signal (IC series, decay, turnover, artifact paths); see `validation_bundle.py` and reportv2.
...
**Failure modes**
- Misaligned signal/return indices → NaN IC or wrong metrics; detect via assert_monotonic_time_index and alignment checks (warn or exit depending on strict mode).
- Too few assets per timestamp → NaN IC at that time; detect via IC series NaNs.
...
- Promotion (candidate → accepted) and Experiment Registry & Manifests are part of Reporting; see dependency_graph and phased_execution.

**Required invariants**
- Every run must be traceable to git_commit, environment fingerprint, dataset fingerprint/id, and output hashes.

**Where errors are handled vs surfaced**
- Missing or corrupt artifacts → report may be partial; strict-integrity can cause exit non-zero.

**Failure modes**
- Missing dataset_id or manifest → traceability broken; detect via manifest and experiment row checks.
- Non-deterministic output hashes on rerun → detect via deterministic rerun test (CRYPTO_ANALYZER_DETERMINISTIC_TIME).

### Execution realism

- **ExecutionCostModel** (`execution_cost.py`): fee, slippage, optional spread_vol_scale and participation impact; all cost application paths delegate here. Used by portfolio.apply_costs_to_portfolio and backtest.
