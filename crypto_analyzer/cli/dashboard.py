...
# Repo root for default DB (crypto_analyzer/cli/dashboard.py -> parents[2] = repo)
DB_PATH_DEFAULT = str(Path(__file__).resolve().parents[2] / "dex_data.sqlite")
...
def main(argv: Optional[List[str]] = None) -> int:
    """Run this dashboard via streamlit."""
    if argv is None:
        argv = sys.argv[1:]
    path = Path(__file__).resolve()
    return subprocess.run([sys.executable, "-m", "streamlit", "run", str(path)] + argv).returncode
